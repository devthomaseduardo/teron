...
export function getMpAccessToken() {
  ...
  if (!token) {
    throw new Error(
      'MERCADOPAGO_ACCESS_TOKEN não configurada. Adicione no .env.local (Access Token de teste ou produção).'
    )
  }
  return token
}
...
/** Converte string tipo "R$ 48.000" ou "48000" em número (BRL). */
export function parseInvestmentBRL(value: string): number {
  ...
  if (!Number.isFinite(n) || n <= 0) {
    throw new Error(`Valor de investimento inválido: ${value}`)
  }
...
export type CreatePreferenceInput = {
  ...
  amount: number
  payerEmail: string
  /** Fração do total cobrada como sinal (0–1). Default 0.3 (30%). */
  signalRatio?: number
}
